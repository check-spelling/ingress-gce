...
func TestGetHealthCheck(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			desc:                 "Get nonexistent global health check",
			existingHealthChecks: []*composite.HealthCheck{regionalHealthCheck, globalHealthCheck},
			getHCName:            "nonexistent-hc",
			getHCScope:           meta.Global,
			expectedHealthCheck:  nil,
		},
		{
			desc:                 "Get nonexistent regional health check",
			existingHealthChecks: []*composite.HealthCheck{regionalHealthCheck, globalHealthCheck},
			getHCName:            "nonexistent-hc",
...
func TestDeleteHealthCheck(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			desc:                        "Delete nonexistent healthCheck",
			existingHealthChecks:        []*composite.HealthCheck{regionalHealthCheck, globalHealthCheck},
			deleteHCName:                "nonexistent",
